...
import { assert, isOldIE } from '../utils';

// Defines poly.gl "feature" names and semantics
...
// capability can be a WebGL extension name or a poly capability name
...
export function getFeatures( gl ) {
    ...
    if ( !gl.poly.caps ) {
        gl.poly.caps = {};
        gl.poly.caps.webgl2 = isWebGL2( gl );
        for ( const cap in WEBGL_FEATURES ) {
            gl.poly.caps[ cap ] = getFeature( gl, cap );
        }
