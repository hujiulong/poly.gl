...
function initializeExtensions( gl ) {
    ...
    for ( const extension of EXTENSIONS ) {
        gl.poly[ extension ] = gl.getExtension( extension );
    }
}
...
export default function polyfillContext( gl ) {
    ...
    if ( !gl.poly.polyfilled ) {
        for ( const extension in WEBGL_CONTEXT_POLYFILLS ) {
            if ( extension !== 'overrides' ) {
                polyfillExtension( gl, { extension, target: gl.poly, target2: gl } );
            }
        }
        installOverrides( gl, { target: gl.poly, target2: gl } );
        gl.poly.polyfilled = true;
    }
    return gl;
}
