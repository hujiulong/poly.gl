...
const log = {
    ...
    log( priority, arg, ...args ) {
        if ( priority <= log.priority ) {
            console.debug( `poly.gl: ${arg}`, ...args );
        }
    },
    info( priority, arg, ...args ) {
        if ( priority <= log.priority ) {
            console.log( `poly.gl: ${arg}`, ...args );
        }
    },
    ...
    warn( arg, ...args ) {
        if ( !cache[ arg ] ) {
            console.warn( `poly.gl: ${arg}`, ...args );
            cache[ arg ] = true;
        }
    },
    error( arg, ...args ) {
        console.error( `poly.gl: ${arg}`, ...args );
    },
    image( {
        priority,
        image,
...
in a later version. Use \`${newUsage}\` instead` );
   ...
   check our Upgrade Guide for more details` );
    ...
    group( priority, arg, {
        ...
        if ( priority <= log.priority ) {
            if ( collapsed ) {
                console.groupCollapsed( `poly.gl: ${arg}` );
            } else {
                console.group( `poly.gl: ${arg}` );
            }
        }
    },
    groupEnd( priority, arg ) {
        if ( priority <= log.priority ) {
            console.groupEnd( `poly.gl: ${arg}` );
        }
    },
    time( priority, label ) {
        if ( priority <= log.priority ) {
