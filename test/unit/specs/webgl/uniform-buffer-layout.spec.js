...
import { GL, UniformBufferLayout, Buffer, Program } from 'poly.gl';
...
it( 'WebGL#UniformLayout', done => {
    ...
    assert.ok( value, 'Std140Layout correct' );

    done();
} );

it( 'WebGL#UniformLayout getData', done => {
    const { gl2 } = fixture;
    if ( !gl2 ) {
    ...
    assert.equal( float1[ 0 ], FLOAT, 'float scalar uniform values matched' );
    assert.deepEqual( vec2, VEC2, 'vec2 uniform values matched' );
    assert.deepEqual( vec3, VEC3, 'vec3 uniform values matched' );
    assert.deepEqual( vec4, VEC4, 'vec4 uniform values matched' );

    done();
} );

// it( 'WebGL#UniformLayout setData', done => {
//     const { gl2 } = fixture;
//     if ( !gl2 ) {
//         assert.ok( true, 'WebGL2 not available, skipping tests' );
//         done();
...
//     assert.deepEqual( outData, expectedData, 'Should receive expected data' );
//
//     done();
// } );
